...
class SWGATLayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        z = self.fc(h)
        g.nodes[snode_id].data['z'] = z
        g.apply_edges(self.edge_attention, edges=swedge_id)  # 原文 式5  apply_edges() 不涉及消息传递下，逐边计算
        g.pull(wnode_id, self.message_func, self.reduce_func)  # 原文 式6  pull 只对wnode_id进行消息传递， update_all全体
        g.ndata.pop('z')
        h = g.ndata.pop('sh')
        return h[wnode_id]
